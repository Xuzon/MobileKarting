...
namespace KartGame.Track
{
    ...
    public class TrackManager : MonoBehaviour
    {
        ...
        public const string BEST_TIME_KEY = "BestTime";
        ...
        [SerializeField]
        [Tooltip("Canvas to enable ond end race")]
        protected GameObject endCanvas;
        ...
        void RacerHitCorrectCheckpoint (IRacer racer, Checkpoint checkpoint)
        {
            if (checkpoint.isStartFinishLine)
            {
                ...
                if (racerCurrentLap > 0)
                {
                    ...
                    float historicalBest = PlayerPrefs.HasKey(BEST_TIME_KEY) ? PlayerPrefs.GetFloat(BEST_TIME_KEY) : -1;
                    if(historicalBest > lapTime || historicalBest == -1)
                    {
                        NewRecord(racer, lapTime);
                    }
        ...
        private void NewRecord(IRacer racer, float lapTime)
        {
            PlayerPrefs.SetFloat(BEST_TIME_KEY, lapTime);
            OnNewRecord?.Invoke();
        }
